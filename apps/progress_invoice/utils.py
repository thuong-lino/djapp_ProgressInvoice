...
def compare_progress_invoice(cch_invoice: dict,  *args, **kwargs):
    ...
    if created:
        ...
        [ProgessInvoiceAllocation.objects.update_or_create(
            progress_invoice=_ref, project_ident=str(item['project_ident']), project_name=item['project_name'], defaults=item) for item in new_allocs]
        current_app_projects = ProgessInvoiceAllocation.objects.filter(
            progress_invoice=_ref)
        current_cch_projects = [str(item['project_ident'])
                                for item in new_allocs]
        project_dsp = current_app_projects.exclude(project_name='Unassigned').exclude(
